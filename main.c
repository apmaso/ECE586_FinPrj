...
int main(int argc, char* argv[]) {
	...
	//TEST2.txt
	//Test3.txt
	//TEST4.txt -same as test3 but has a branch (BEQ) in the middle
	//addi_sw.txt
	//ld_st.txt
	//load1.txt
	//sw_lw.txt
...
void fetch_instruction(int *pc){
	...
	if(flush==1){
		pipeline[wrong_path_inst[1]].pipe_stage=-1;
	}
	else{
		(*pc)++;
	}
	...
	if (fetched_inst_stage == -1 || *pc == 1) {
		...
		if(branch_encountered==1){
			wrong_path_inst[0]=empty_pipe_slot;	
			branch_encountered++;
		}
		else if(branch_encountered==2){
			wrong_path_inst[1]=empty_pipe_slot;
			branch_encountered++;
		}	
/*		if(branch_encountered==3){
			branch_encountered=0;
		}
*/
		if((pipeline[empty_pipe_slot].opcode==14 || pipeline[empty_pipe_slot].opcode==15 || pipeline[empty_pipe_slot].opcode==16) && branch_encountered==0){
	...
	printf("********************wrong_path 0: %d\n", wrong_path_inst[0]);
	printf("********************wrong_path 1: %d\n", wrong_path_inst[1]);
	printf("^^^^^^^^^^^^^^^^^^^^branch encountered: %d\n", branch_encountered);
...
void execute_instruction(int *pc){
		...
		if(stagebusy(2)==-1 && decoded_inst_index != -1){
				printf("branch encountered is: %d\n", branch_encountered);
				if(pipeline[decoded_inst_index].TYPE==0 && branch_encountered<3) {
				...
				if(pipeline[decoded_inst_index].TYPE==1 && branch_encountered<3) {
					...
					} else if(pipeline[decoded_inst_index].opcode==16) {
						
						pipeline[decoded_inst_index].pipe_stage=2;//update execution stage of instruction in pipe.
					
						branch_encountered=0;
						//(*pc)=pipeline[decoded_inst_index].rs; FIXME: this needs to be uncommented.
						(*pc)=7;//FIXME: address needs to be multiplied by four and handled in fetch to divide by four.
						flush=1;
						pipeline[wrong_path_inst[0]].pipe_stage=-1;
						pipeline[wrong_path_inst[1]].pipe_stage=-1;
...
void memory_access(){
	...
	if(stagebusy(3) == -1 && executed_inst_index != -1){
			...
			if(pipeline[executed_inst_index].opcode==12 /*&& no store with same address is pending*/ ){//type 1.
				...
				reg_ready[pipeline[executed_inst_index].rt]=cycle+1;
				...
				if(cycle>reg_ready[pipeline[executed_inst_index].rt]){
					...
					//FIXME address conflicts need to be resolved.
					data_red[effective_address]=reg_ready[pipeline[executed_inst_index].rs]+1;
					pipeline[executed_inst_index].pipe_stage=3;
