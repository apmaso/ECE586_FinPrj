...
void execute_instruction(){
		...
		printf("#########reg_ready[1] is %d\n", reg_ready[1]);
		int decoded_inst_index=stagebusy(1);
		if(stagebusy(2)==-1 && decoded_inst_index != -1){

				//printf("inside decode's IF\nfetched_inst_index is: %d\n", fetched_inst_index);
				if(program[decoded_inst_index].TYPE==0) {

					if(cycle>reg_ready[pipeline[decoded_inst_index].rs] && cycle>reg_ready[pipeline[decoded_inst_index].rt]){

						printf("rt $$$$$$$$$$$$$$$$$$$$$$$$$$ %d\n",reg_ready[pipeline[decoded_inst_index].rt]);
						//FIXME: check potential bug if execution has a stall.  so ready will bew more plan 2 cycles later.	
				...
				if(program[decoded_inst_index].TYPE==1) {

					if(cycle>reg_ready[pipeline[decoded_inst_index].rs]) {
						printf("$$$$$$$$$$$$$$$$$$$$$$$$$$ %d\n",reg_ready[pipeline[decoded_inst_index].rs]);
		...
		//printf("#########reg_ready[1] is %d\n", reg_ready[1]);
		//printf("#########reg_ready[10] is %d\n", reg_ready[10]);
		for(int i=0;i<32;i++){
			printf("^^^^reg_ready[%d] is %d\n", i, reg_ready[i]);
		}
