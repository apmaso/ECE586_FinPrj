...
void print_stuff(int bin_inst[]);

struct instruction2 program[500]; //TODO FIXME
...
void print_inst(struct instruction2 instr){
...
void print_stuff(int bin_inst[]){
	int type = extract_type(bin_inst);

	int op=extract_opcode(bin_inst, 6);
	int rs=extract_rs(bin_inst);
	int rt=extract_rt(bin_inst);
	int rd, imm; 
	if(type==0)
		rd=extract_rd(bin_inst);
	if(type==1)
		imm=extract_immediate(bin_inst);
	...
	if(op==2)
		strcpy(inst_str, "SUB");
	if(op==3)
		strcpy(inst_str, "SUBI");
	if(op==4)
		strcpy(inst_str, "MUL");
	if(op==5)
		strcpy(inst_str, "MULI");
	if(op==6)
		strcpy(inst_str, "OR");
	if(op==7)
		strcpy(inst_str, "ORI");
...
struct instruction2 fill_instruction(int bin_inst[], int len){

	struct instruction2 my_instruct;
